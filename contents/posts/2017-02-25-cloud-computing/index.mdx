...
1.  Degree centrality  
    ...
    ![degreeCentrality](/images/posts/2017-02-25-cloud-computing/2017-02-25-degreeCentrality01.png)
    但 degree centrality 的缺點是如果有一個點的 degree 很低,
    但是這個點連接了多個 community,
    用 degree centrality 看不出這個點很重要

![degreeCentrality](/images/posts/2017-02-25-cloud-computing/2017-02-25-degreeCentrality02.png)
...
![clique img](/images/posts/2017-02-25-cloud-computing/2017-02-25-community01.png)
...
Input : weighted graph Output : 切成兩個 equal-size subgraph,且橫跨兩群的 crossing edge 的總和最小
...
## Hiraichiecal Clustering

bottom-up 每一回合都找兩個最像的做合併 _ single link 距離取 min _ complete link 距離取 max
...
## Topological Pattern Based Methods 計算兩點之間的分數,若大於某個值就表示他們之間有關係(連線)
...
將沒有 label 的 node 用 Ghost edge 連到有 label 的 node,有 label 的 node 就算沒有實體的 edge 連到 unlabel node 但是也可以造成影響。且影響力可以依據距離來做調整。
...
![Information Diffusion](/images/posts/2017-02-25-cloud-computing/2017-02-25-InformationDiffusion.png)
